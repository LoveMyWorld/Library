...
<div class="container">
  ...
  <div class="content-box">
    ...
    <div class="toolbar">
      ...
      <div class="search">
        <form action="${pageContext.request.contextPath}/CollectionServlet" method="get">
    ...
    <table>
      ...
      <%
        ...
        int count = 1; // 初始化计数器
        if (collectionList != null) { // 判断数据是否为空
          for (Collection collection : collectionList) {
      %>
      <tr >
        <td><%= count++ %></td>
        <td><%= collection.getName() %></td>
        <td><%= collection.getAddr() %></td>
        <td><%= collection.getContact() %></td>
        <td><%= collection.getPhoneNum() %></td>
        <td><%= collection.getPostcode() %></td>
    ...
    <div class="pagination">
      <div class="pagination">
        <!-- 上一页 -->
        <button onclick="location.href='${pageContext.request.contextPath}/CollectionServlet?currentPage=<%= currentPage - 1 %>'">&laquo; 上一页</button>
        <!-- 当前页信息 -->
        <span>第 <%= currentPage %> / <%= totalPages %> 页，每页显示 16 条</span>
        <!-- 下一页 -->
        <button onclick="location.href='${pageContext.request.contextPath}/CollectionServlet?currentPage=<%= currentPage + 1 %>'">下一页 &raquo;</button>
      </div>
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    submitForm.onclick = function () {
      ...
      if (confirmSubmit) {
        ...
        var formData=$('#collectionForm_add').serialize();
        // console.log("即将开始Ajax");
        // console.log(formData );
        // 使用 AJAX 发送数据到后端
        $.ajax({
          url: '${pageContext.request.contextPath}/AddCollectionServlet',  // 后端接口，用于提交数据
          method: 'POST',
    ...
    submitForm2.onclick = function () {
      ...
      if (confirmSubmit) {
        ...
        var formData=$('#collectionForm_edit').serialize();
        // console.log("即将开始Ajax");
        // console.log(formData );
        // 使用 AJAX 发送数据到后端
        $.ajax({
          url: '${pageContext.request.contextPath}/EditCollectionServlet',  // 后端接口，用于提交数据
          method: 'POST',
          data: formData,  // 发送的表单数据
          dataType: 'json',  // 期待返回的数据格式
          success: function(response) {
  ...
  // 获取并填充读者详细信息
  function fetchCollectionDetails(name) {
    // 假设我们通过后端接口 `/LookCollectionServlet` 获取数据
    $.ajax({
      url: '${pageContext.request.contextPath}/LookCollectionServlet', // 你的后端接口
      method: 'GET',
      ...
      error: function(xhr, status, error) {
        alert("获取收藏单位信息失败，错误代码: " + xhr.status + "\n" + xhr.statusText);
      }
    });
  }
    ...
    deleteButtons.forEach(function(button) {
      button.addEventListener("click", function(event) {
        ...
        if (confirmSubmit) {
          ...
          $.ajax({
            url: '${pageContext.request.contextPath}/DeleteCollectionServlet',  // 后端接口，用于提交数据
            method: 'POST',
